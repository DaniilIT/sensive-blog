...
def serialize_post_optimized(post):
    return {
        ...
        'published_at': post.published_at,
        'slug': post.slug,
        'tags': [serialize_tag_optimized(tag) for tag in post.tags.all().annotate(count_posts=Count('posts'))],
        'first_tag_title': post.tags.all()[0].title,
    }
...
def index(request):
    ...
    most_popular_tags = Tag.objects.popular().annotate(count_posts=Count('posts'))[:5]
